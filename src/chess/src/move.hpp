...
class Move {
   public:
    Move() = default;
    Move(std::uint16_t move) : move_(move), score_(0) {}
    ...
    const static std::uint16_t NO_MOVE   = 0;
    const static std::uint16_t NULL_MOVE = 65;
    const static std::uint16_t NORMAL    = 0;
    const static std::uint16_t PROMOTION = 1 << 14;
    const static std::uint16_t ENPASSANT = 2 << 14;
    const static std::uint16_t CASTLING  = 3 << 14;
