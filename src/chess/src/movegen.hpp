...
static std::array<std::array<Bitboard, 64>, 64> init_squares_between() {
    ...
    std::array<std::array<Bitboard, 64>, 64> SQUARES_BETWEEN_BB = init_squares_between();
...
[[nodiscard]] static Bitboard checkMask(const Board &board, Square sq, int &double_check) {
    ...
    if (bishop_attacks) {
        ...
        mask |= chess::movegen::SQUARES_BETWEEN_BB[sq.index()][index] | Bitboard::fromSquare(index);
        double_check++;
    }
...
[[nodiscard]] static Bitboard pinMaskRooks(const Board &board, Square sq, Bitboard occ_opp, Bitboard occ_us) {
    ...
    while (rook_attacks) {
        ...
        const Bitboard possible_pin = chess::movegen::SQUARES_BETWEEN_BB[sq.index()][index] | Bitboard::fromSquare(index);
...
static void generatePawnMoves(const Board &board, Movelist &moves, Bitboard pin_d, Bitboard pin_hv,
    ...
    const Direction DOWN_LEFT       = c == Color::WHITE ? Direction::SOUTH_WEST : Direction::NORTH_EAST;
    const Direction DOWN_RIGHT      = c == Color::WHITE ? Direction::SOUTH_EAST : Direction::NORTH_WEST;
    const Bitboard RANK_B_PROMO     = c == Color::WHITE ? attacks::MASK_RANK[static_cast<int>(Rank::RANK_7)]
                                                            : attacks::MASK_RANK[static_cast<int>(Rank::RANK_2)];
    const Bitboard RANK_PROMO       = c == Color::WHITE ? attacks::MASK_RANK[static_cast<int>(Rank::RANK_8)]
                                                            : attacks::MASK_RANK[static_cast<int>(Rank::RANK_1)];
    const Bitboard DOUBLE_PUSH_RANK = c == Color::WHITE ? attacks::MASK_RANK[static_cast<int>(Rank::RANK_3)]
...
[[nodiscard]] static Bitboard generateCastleMoves(const Board &board, Square sq, Bitboard seen,
    ...
    for (const auto side : {Board::CastlingRights::Side::KING_SIDE, Board::CastlingRights::Side::QUEEN_SIDE}) {
        ...
        const Bitboard not_occ_path       = chess::movegen::SQUARES_BETWEEN_BB[sq.index()][from_rook_sq.index()];
        const Bitboard not_attacked_path  = chess::movegen::SQUARES_BETWEEN_BB[sq.index()][end_king_sq.index()];
...
/// @param board
template <Color::underlying c, MoveGenType mt>
static void legalmoves(Movelist &movelist, const Board &board, int pieces) noexcept {
    /*
    ...
    */
    ...
    if (pieces & PieceGenType::KING) {
        Bitboard seen;
        if (c == Color::WHITE) {
            seen = seenSquares<Color::BLACK>(board, opp_empty);
        } else {
            seen = seenSquares<Color::WHITE>(board, opp_empty);
        }
