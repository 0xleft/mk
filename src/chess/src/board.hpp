...
class Board {
   ...
   public:
    class CastlingRights {
       public:
        ...
        template <typename T>
        static Side closestSide(T sq, T pred) {
   ...
   public:
    explicit Board(bpstd::string_view fen = constants::STARTPOS) { setFenInternal(fen); }
    void setFen(bpstd::string_view fen) { setFenInternal(fen); }
    ...
    typename std::enable_if<std::is_same<T, PieceType>::value, T>::type at(Square sq) const {
   ...
   private:
    ...
    /// @param fen
    void setFenInternal(bpstd::string_view fen) {
        original_fen_ = fen;
        ...
        for (char curr : position) {
            auto piece_str = bpstd::string_view(&curr, 1);
    ...
    bpstd::string_view original_fen_;
};
}  // namespace  chess
